...
const BlockCard = () => {
  ...
  const { data: getAllTopBlocksPlayer, isLoading } = useSWR(Endpoint.TOP_BLOCK, fetcher);
    ...
    <Card className="bg-[rgb(36,36,36)] border-0">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <p className="text-white text-md">Blocks</p>
      ...
      <CardContent className="flex flex-col space-y-5">
        ...
        {isLoading ? (
            ...
            getAllTopBlocksPlayer?.slice(0,5).map((player: any, index: number, array: any[]) => (
              <div key={index} className="flex flex-col space-y-2">
                <div className="text-white flex items-center justify-between text-center">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>{roundFigure(player?.avg_blocks)}</div>
                </div>
