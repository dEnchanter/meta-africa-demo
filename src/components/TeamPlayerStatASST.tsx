...
const TeamPlayerStatASST = ({ id, logoSrc, name, position, team, statValue }: PlayerCardProps) => {
  ...
  return (
    <div onClick={() => router.push(`/dashboard/players/${id}`)} className="flex items-center space-x-2 cursor-pointer">
      ...
      <div className="text-xl font-semibold">{`${roundFigure(statValue)} AST`}</div>
    </div>
  )
}

export default TeamPlayerStatASST